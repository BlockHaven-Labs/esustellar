...
| **Monitoring stack** | `infra/monitoring/` (Prometheus, Grafana, Loki, Promtail, Alertmanager) | `monitoring/` (root: `loki-config.yaml`, `promtail-config.yaml`, `balance-exporter/`), `k8s/monitoring/` | `infra/monitoring/` holds the full deployable stack. The root `monitoring/` dir and `k8s/monitoring/` manifests predate it and are kept only because `scripts/validate-monitoring-config.sh` and `docs/logging.md` still reference them. |
