...
fn test_update_group_info_updates_metadata() {
    ...
    client.update_group_info(&group, &admin, &new_name, &false, &10);
...
// ── Issue #40: min-length validation on group_id and name ─────────────────────

#[test]
#[should_panic(expected = "Error(Contract, #105)")]
fn test_register_group_empty_group_id_is_rejected() {
    let env = setup_env();
    let client = create_registry(&env);
    let admin = Address::generate(&env);
    let contract = Address::generate(&env);

    // Empty group_id must be rejected with InvalidInput (105).
    client.register_group(
        &contract,
        &String::from_str(&env, ""),
...
#[test]
#[should_panic(expected = "Error(Contract, #105)")]
fn test_register_group_empty_name_is_rejected() {
    let env = setup_env();
    let client = create_registry(&env);
    let admin = Address::generate(&env);
    let contract = Address::generate(&env);

    // Empty name must be rejected with InvalidInput (105).
    client.register_group(
        &contract,
...
#[test]
#[should_panic(expected = "Error(Contract, #105)")]
fn test_update_group_info_empty_name_is_rejected() {
    let env = setup_env();
    let client = create_registry(&env);
    let admin = Address::generate(&env);

    let group = register_group(&env, &client, "g-upd-empty", "Original Name", &admin, true, 5);

    // Updating to an empty name must be rejected with InvalidInput (105).
...
#[test]
fn test_register_group_minimal_valid_id_and_name_accepted() {
    // Single-character id and name are at the minimum threshold — must succeed.
    let env = setup_env();
    let client = create_registry(&env);
    let admin = Address::generate(&env);
    let contract = Address::generate(&env);
    ...
    let info = client.get_group_info(&contract);
    assert_eq!(info.group_id, String::from_str(&env, "a"));
    assert_eq!(info.name, String::from_str(&env, "B"));
}
